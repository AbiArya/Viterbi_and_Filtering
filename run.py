...
def filtering(actions,evidence,map,probgrid,count):#a row will be the y, and the columns will be x. 2d arrays are [row][col] [y][x]
    ...
    for count1, a in enumerate(newgrid):
        for count2, b in enumerate(a):
            newgrid[count1][count2]*=alpha
    avg=avg/9000
    return newgrid,avg,maxx,maxy
...
def runprog():
    ...
    for i in range(10):
        ...
        for q in range(10):
            ...
            count = 0
            for a in range(100):
                probgrid,avg,maxx,maxy=filtering(actions,evidence,tmpmap,probgrid,count)
                currx1,curry2=getLoc(str(i)+str(q),count)
                currx1=int(float(currx1))
                curry2=int(float(curry2))
                curdist.append(dist(currx1,curry2,maxx,maxy))
                probs.append(probgrid[curry2][currx1])
                count+=1
            totprobs.append(probs)
    ...
    for z in range(100):
        ...
        for p in range(100):
            ...
            tmpprob+=totprobs[p][z]
        finaldist.append(tmpdist/100)
        finalprob.append(tmpprob/100)
    print(finaldist)
    print(finalprob)
...
def viterbi(actions,evidence,map,probgrid,count):#a row will be the y, and the columns will be x. 2d arrays are [row][col] [y][x]
    ev=evidence[count]
    act=actions[count]
    max=0
    maxx=0
            ...
            for count2, x in enumerate(y):#iterates cols/x
                if(map[count1][count2]!='b'):
                    if(count1==0):
                        ...
                        if(stuff>newval):
                            tmpgridrow.append(stuff)
                            paths[count1][count2].append((count1-1,count2))
                        else:
                            paths[count1][count2].append((count1,count2))
                            tmpgridrow.append(newval)
                        avg+=newval
                        if (newval > max):
                            max = newval
...
map1=takeMap(int(mapnum))
#runprog()RUN PROG
#generateData(map1)
groundnum=input("What ground state #?")
...
for i in range(0):
    tmptime=time.time()
    probgrid,avg,maxx,maxy=filtering(actions,evidence,map1,probgrid,count)
...
while not done:
    ...
    for val in alllocs:
        if(val[0] != str(currx) and val[1] != str(curry) and val[0] != str(maxx) and val[1] != str(maxy)):
            pygame.draw.rect(screen, (0, 255, 0),[(MARGIN + WIDTH) * int(val[0]) + MARGIN, (MARGIN + HEIGHT) * int(val[1]) + MARGIN, WIDTH, HEIGHT])
            #pass
